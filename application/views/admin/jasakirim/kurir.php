<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-4 font-weight-bold text-dark">Data Kurir</h1>
    ...
    <div class="card shadow-sm border-0 rounded-lg col-lg-8 px-0">
        <div class="card-header d-flex justify-content-between align-items-center"
            ...
            <h6 class="mb-0 font-weight-bold">List Kurir</h6>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0" id="dataTable">
                    ...
                    <tbody>
                        <?php $n = 1; foreach ($kurir as $key) : ?>
                        <tr>
                            <td class="text-center align-middle"><?= $n++ ?></td>
                            <td class="align-middle"><?= htmlspecialchars($key->nama_kurir) ?></td>
                            <td class="text-center align-middle">
                                <button class="btn btn-sm btn-warning rounded-pill mr-1" data-toggle="modal"
                                    data-target="#editModal<?= $key->id_kurir ?>">
                                    Edit
                                </button>
                                <a href="<?= site_url('jasakirim/delete_kurir/' . $key->id_kurir) ?>"
...
<div class="modal fade" id="tambahModal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="<?= site_url('jasakirim/add_kurir') ?>" method="POST">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="nama_kurir" placeholder="Nama Kurir" required>
                    </div>
...
<div class="modal fade" id="editModal<?= $key->id_kurir ?>">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="<?= site_url('jasakirim/kurir_save_edit') ?>" method="POST">
                <div class="modal-body">

                    <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>"
                        value="<?= $this->security->get_csrf_hash() ?>">
                    <input type="hidden" name="id_kurir" value="<?= $key->id_kurir ?>">

                    <div class="form-group">
                        <label>Nama Kurir</label>
                        <input type="text" class="form-control" name="nama_kurir"
                            value="<?= htmlspecialchars($key->nama_kurir) ?>" required>
                    </div>
